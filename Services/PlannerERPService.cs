...
namespace Sanibell_ProductionModule.Services
{
    public class PlannerErpService : IPlannerErpService
    {
        ...
        // processes the production order in the ERP system
        public async Task ProductionOrderVerwerkenAsync(string productieorderNummer)
        {
            var baseUrl = _config["PlannerERPSettings:BaseUrl"]?.TrimEnd('/');
            var token = _config["PlannerERPSettings:Process_Order_Secret"];
            var relativePath = "Productieorder_Verwerken";
            ...
            Console.WriteLine($"Verwerk Order response: {response.StatusCode} - {responseBody}");
